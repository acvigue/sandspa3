...
export const useFilesStore = defineStore("files", {
    ...
    playlists: [
      {
        ...
        description:
          "November  2022 Community Track Submissions Our Staff Loves",
        db_tracks: [],
        created_by_name: "Sam",
  ...
  actions: {
    async refreshFiles() {
      ...
      await axios
        .get(`${main.tableBaseURL}/files/sd/manifest.json`)
        .then((response) => {
    ...
    async addPlaylist(playlist) {
      ...
      console.log(
        `Playlist manifest downloaded! ${playlistData.length} items to process.`
      );
      ...
      for (const item of playlistData) {
        if (item.type == "track") {
          ...
          if (
            this.tracks.find((trackobj) => trackobj.id === item.id) == undefined
          ) {
          ...
          formData.append("file", blob, `${itm.id}.seq`);
          await axios.post(`${main.tableBaseURL}/uploadtofileman`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
